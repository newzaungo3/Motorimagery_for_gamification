    ...
    while True:
        ...
        if 'space' in keys:     
            ...
            for session in range(NUM_SESSION):
                image_list , video_list = randomStimuli(NUM_TRIAL)
                for block in range(NUM_BLOCK):
                    ...
                    for trials in range(NUM_TRIAL):
                        ...
                        if PLAY_VIDEO == True:
                            ...
                            playVideo(f"{stim[trials]}",BLOCK_MARKER[stim[trials]],STIM_TIME,board_shim,mywin)
                            #b.hear('A_')
                            drawFixation(FIXATION_TIME,board_shim,mywin)
                            STIM_CHECK += 1
                            print(STIM_CHECK)
                        else:
                            drawTrial(f"{stim[trials]}",BLOCK_MARKER[stim[trials]],STIM_TIME,board_shim,mywin)
                            drawFixation(FIXATION_TIME,board_shim,mywin)
                            STIM_CHECK += 1
                            print(STIM_CHECK)

...
def drawBaselinerun(openTime,closeTime,board,board_id):
    baseline_open_name = f'{PARTICIPANT_ID}R01'
    #Baseline run
    #open
    ...
    data_copy = data.copy()
    raw_open = getdata(data_copy,board_id,n_samples = 250)
    save_raw(raw_open,baseline_open_name)   
    #close
    baseline_close_name = f'{PARTICIPANT_ID}R02'
    ...
    data = board.get_board_data()
    data_copy = data.copy()
    raw_close = getdata(data_copy,board_id,n_samples = 250)
    save_raw(raw_close,baseline_close_name)
...
def main():
    ...
    while True:
        ...
        if 'space' in keys:      # If space has been pushed
            start = time.time()
            drawTextOnScreen('') 
            drawBaselinerun(BASELINE_EYEOPEN,BASELINE_EYECLOSE,board_shim,args.board_id)
            #experiment      
            #3 session

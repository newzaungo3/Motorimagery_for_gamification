...
def drawBaselinerun(openTime,closeTime,board,board_id):
    baseline_open_name = f'{PARTICIPANT_ID}R00'
    #Baseline run
    #open
    ...
    #close
    baseline_close_name = f'{PARTICIPANT_ID}R01'
        ...
        if 'space' in keys:      # If space has been pushed
            ...
            for session in range(NUM_SESSION):
                ...
                for block in range(NUM_BLOCK):
                    ...
                    #save mne when complet 2 block(executed and imagine)
                    if BLOCK_COUNT % 2 == 0:
                        block_name = f'{PARTICIPANT_ID}R0{BLOCK_COUNT + 1}' 
                        data = board_shim.get_board_data()
                        data_copy = data.copy()
                        raw = getdata(data_copy,args.board_id,n_samples = 250)

...
def save_fig(data,name,trials):
    folder_path = create_session_folder(PARTICIPANT_ID,ERD_FOLDER)
    data.savefig(os.path.join(folder_path, f'{name}{TYPE_OF_FILE}'))
    return os.path.basename(folder_path)
...
def Erd_Plot(epochs,trial):
    ...
    file = f'{PARTICIPANT_ID}R{EXECUTE_NO[EXE_COUNT]:02d}'
    g.savefig(f'.\erd\{NAME}_{trial}.png')

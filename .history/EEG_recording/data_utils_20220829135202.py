...
def save_raw_to_dataframe(raw,name):
    ...
    csv_folder = create_session_folder(PARTICIPANT_ID)
    csv_name = f'{name}.csv'
    epoch_dataframe.to_csv(os.path.join(csv_folder,csv_name),encoding='utf-8')

def save_raw(raw, name):
    folder_path = create_session_folder(PARTICIPANT_ID)
    raw.save(os.path.join(folder_path, f'{name}.fif'))
    return os.path.basename(folder_path)

def create_session_folder(subj):
    folder_name = f'{subj}'
    if os.path.isdir(os.path.join(RECORDING_DIR, folder_name)):
        folder_path = os.path.join(RECORDING_DIR, folder_name)
    else:
        folder_path = os.path.join(RECORDING_DIR, folder_name)
        Path(folder_path).mkdir(exist_ok=True,overwrite=True)
    return folder_path

    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        for trials in range(NUM_TRIAL):
            ...
            if IS_ONLINE == True:
                ...
                data_copy = data.copy()
                #sending and predict
                raw = getdata(data_copy,BOARD_ID,n_samples = 250,dropEnable = True)
                ...
                print(type(train_epochs))
                print(epochs_raw_data.event_id)
                print('-------------------------------------------------')
                print('Sending')
                drawTextOnScreen('Sending',mywin)
                #temp:ndarray = data_copy.copy()
                temp:ndarray = train_epochs.copy()
                label_temp:ndarray = labels.copy()
                database = Database(train=temp,label=label_temp,names=file_name)

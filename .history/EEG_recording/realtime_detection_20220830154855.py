...
from data_utils import getdata,getepoch
...
def main():
    ...
    try:
        ...
        #Predict
        while board_shim.is_prepared():
            #Get eeg data from board without remove buffer
            data = board_shim.get_current_board_data(250)
            print(data.shape)
            #get raw and epoch data
            raw = getdata(data,args.board_id,n_samples = 250)
            train_epochs,_,_= getepoch(raw,4,10)

...
def save_dataframe(datafram,name):
    csv_folder = create_session_folder(name)
    csv_name = f'{now_datestring()}_{name}.csv'
    datafram.to_csv(os.path.join(csv_folder,csv_name),encoding='utf-8')
...
def getdata(data,board,clear_buffer=False,n_samples=None):
    ...
    raw_dataframe = raw_data.copy().to_data_frame()
    #2 electrode
